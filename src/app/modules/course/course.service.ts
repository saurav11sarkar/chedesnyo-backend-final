...
const getAllCourse = async (params: any, options: IOption) => {
  ...
  const { searchTerm, level, language, minPrice, maxPrice, isPublic, duration, ...filterData } = params;

  const andCondition: any[] = [];
  const searchableFields = ['title', 'description', 'targetAudience'];
  ...
  if (level) andCondition.push({ level });
  if (language) andCondition.push({ language: { $regex: language, $options: 'i' } });
  if (duration) andCondition.push({ duration: { $regex: duration, $options: 'i' } });
  if (isPublic !== undefined) andCondition.push({ isPublic: isPublic === 'true' });
  if (minPrice) andCondition.push({ price: { $gte: Number(minPrice) } });
  if (maxPrice) andCondition.push({ price: { $lte: Number(maxPrice) } });
  ...
  const promotedIds = await Promotion.find({
    targetType: 'course', status: 'active', startDate: { $lte: new Date() }, endDate: { $gte: new Date() },
  }).select('targetId');
  ...
  const sorted = result.sort((a, b) => {
    const aP = promotedSet.has(a._id.toString()) ? 1 : 0;
    const bP = promotedSet.has(b._id.toString()) ? 1 : 0;
    return bP - aP;
  });

  const total = await Course.countDocuments(whereCondition);

  return { data: sorted, meta: { total, page, limit } };
};
