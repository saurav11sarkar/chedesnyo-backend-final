...
const getReferralTree = async (options: IOption) => {
  ...
  const referrers = await User.aggregate([
    ...
    {
      $project: {
        firstName: 1, lastName: 1, email: 1, profileImage: 1, role: 1, referralCode: 1, balance: 1,
        referredCount: { $size: '$referredUsers' },
        referredUsers: {
          $map: {
            input: '$referredUsers',
            as: 'u',
            in: { _id: '$$u._id', firstName: '$$u.firstName', lastName: '$$u.lastName', email: '$$u.email', role: '$$u.role', createdAt: '$$u.createdAt' },
          },
        },
      },
    },
  ...
  const total = await User.countDocuments({ referredBy: { $exists: true, $ne: null } });
  return { data: referrers, meta: { total, page, limit } };
};

const getUserReferrals = async (userId: string) => {
