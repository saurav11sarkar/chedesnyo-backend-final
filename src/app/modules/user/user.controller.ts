...
const getReferralTree = catchAsync(async (req, res) => {
  const options = pick(req.query, ['page', 'limit', 'sortBy', 'sortOrder']);
  const result = await userService.getReferralTree(options);
  sendResponse(res, { statusCode: 200, success: true, message: 'Referral tree fetched', meta: result.meta, data: result.data });
});

const getUserReferrals = catchAsync(async (req, res) => {
  const result = await userService.getUserReferrals(req.params.id);
  sendResponse(res, { statusCode: 200, success: true, message: 'User referrals fetched', data: result });
});
