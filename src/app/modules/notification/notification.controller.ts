...
const adminSendNotification = catchAsync(async (req, res) => {
  ...
  const result = await notificationService.adminSendNotification({ recipientIds, title, body, link });
  sendResponse(res, { success: true, statusCode: 200, message: `Notification sent to ${result.length} users`, data: result });
});
