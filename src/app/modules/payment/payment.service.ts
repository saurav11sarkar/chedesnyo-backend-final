...
import { paymentApprovedTemplate, paymentRejectedTemplate, referralBonusTemplate, courseSoldTemplate } from '../../helper/emailTemplates';
...
const createAssasmtPay = async (userId: string, assasmtId: string) => {
  ...
  const commissionRate = (seller.commissionRate ?? 15) / 100;
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    payment_intent_data: {
      ...
      application_fee_amount: Math.round(amount * commissionRate),
      transfer_data: {
...
const createCoursePay = async (userId: string, courseId: string) => {
  ...
  const courseCommissionRate = (seller.commissionRate !== undefined && seller.commissionRate !== 15) ? seller.commissionRate / 100 : 0.10;
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    payment_intent_data: {
      capture_method: 'manual',
      application_fee_amount: Math.round(amount * courseCommissionRate),
      transfer_data: {
        destination: seller.stripeAccountId,
      },
    },
...
const approveAssasmentPayment = async (
  ...
  const rate = (seller.commissionRate ?? 15) / 100;
  const adminCents = Math.round(totalCents * rate);
  const sellerCents = totalCents - adminCents;
  ...
  if (buyer) {
    await sendMailer(buyer.email, buyer.firstName, paymentApprovedTemplate(buyer.firstName, assasmt.title, payment.amount)).catch(() => {});
  }
  if (buyerUser?.referredBy) {
    const referrer = await User.findById((buyerUser.referredBy as any)._id || buyerUser.referredBy);
    if (referrer) {
      const refBonus = Math.round(adminCents * 0.2) / 100;
      await sendMailer(referrer.email, referrer.firstName, referralBonusTemplate(referrer.firstName, refBonus)).catch(() => {});
    }
  }

  return {
    message: `Payment approved: ${100 - seller.commissionRate}% transferred to seller, ${seller.commissionRate}% kept as platform fee`,
...
const approveCoursePayment = async (paymentId: string, businessId: string) => {
  ...
  const totalCents = captured.amount_received;
  const courseRate = (seller.commissionRate !== undefined && seller.commissionRate !== 15) ? seller.commissionRate / 100 : 0.10;
  const adminCents = Math.round(totalCents * courseRate);
  const sellerCents = totalCents - adminCents;
  ...
  if (buyerUser?.referredBy) {
    ...
    await notificationService.createNotification({
      ...
      type: 'referral_bonus',
      title: 'Referral Bonus Received!',
      body: `You earned €${referralBonus.toFixed(2)} referral bonus from a course sale.`,
    });
  }
  ...
  await notificationService.createNotification({
    ...
    body: `Your payment for course "${course.title}" has been approved. You now have access.`,
    relatedId: payment._id as any,
  });

  // Email notifications
  const courseBuyer = await User.findById(payment.user);
  if (courseBuyer) {
    await sendMailer(courseBuyer.email, courseBuyer.firstName, paymentApprovedTemplate(courseBuyer.firstName, course.title, payment.amount)).catch(() => {});
  }
  await sendMailer(seller.email, seller.firstName, courseSoldTemplate(seller.firstName, course.title, payment.amount)).catch(() => {});

  return {
    message: `Course payment approved: ${Math.round((1 - courseRate) * 100)}% to creator, ${Math.round(courseRate * 100)}% platform fee`,
    sellerAmount: sellerCents / 100,
    adminAmount: adminCents / 100,
  };
};
